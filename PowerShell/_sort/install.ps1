...
foreach ($filename in Get-ChildItem -Path $FolderPath -Filter "dataset*.sql") { Invoke-Sqlcmd -ServerInstance ".\SQLEXPRESS" -Database "localDB" -InputFile $filename}

foreach ($filename in Get-ChildItem -Path $FolderPath -Filter "dataset*.sql") { Invoke-Sqlcmd -ServerInstance ".\SQLEXPRESS" -Database "localDB" -InputFile $filename ; "$filename" }
