...
Parameters explained
    ...
    /FFT is a very important option, as it allows a 2-second difference when comparing timestamps of files, such that minor clock differences between your computer and your backup device dont matter. This will ensure that only modified files are copied over, even if file modification times are not exactly synchronized.
...
Additional useful parameters mentioned by other users
    ...
    /XJD excludes "junction points" for directories, symbolic links that might cause problems like infinite loops during backup. See Brians comments for details.
