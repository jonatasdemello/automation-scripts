...
C:\Users\jonatas\AppData\Roaming\Microsoft\Windows\PowerShell\PSReadLine\ConsoleHost_history.txt
-------------------------------------------------------------------------------------------------------------------------------
...
# note pay attention to the name of the result file, otherwise it gets into a loop!
Get-Content Tokens-*.csv | Out-File -FilePath Merged.csv -Encoding ASCII -Append
...
write-output "workspace-tmp: $env:WORKSPACE_TMP"

write-output "jenkins-home: $env:JENKINS_HOME"
write-output "build-number: $env:BUILD_NUMBER"
