...
# awk is a grep replacement otpmized for source code

sudo apt-get update
sudo apt-get install ack-grep

sudo dpkg-divert --local --divert /usr/bin/ack --rename --add /usr/bin/ack-gr
...
# So the actions are applicable to all the lines.
# Action print without any argument prints the whole line by default,
...
# For each record i.e line, the awk command splits the record delimited by whitespace character by default
